...
1. `.github/workflows/ci.yml` runs every deterministic Pull Request check: branch naming, Docker Compose configuration validation, and Gitleaks.
...
3. Failing branch-name, Compose configuration, or Gitleaks validation leaves the AI workflow unstarted and does not consume provider quota.
