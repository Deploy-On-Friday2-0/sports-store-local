...
def test_reusable_workflow_is_least_privilege_and_data_only():
    for required in ("actions: read", "contents: read", "pull-requests: write"):
        assert required in REUSABLE
    for forbidden in ("id-token: write", "contents: write", "packages: write"):
    ...
    assert "docker compose" not in REUSABLE
    assert "pytest" not in REUSABLE
    assert "review-requirements.txt" in REUSABLE
